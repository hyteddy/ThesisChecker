...
/**
 * @author Viciu Component for extracting Text from Docx files.
 */
public class DocxTextExtractor implements ITextExtractor {
    ...
    /**
     * Creates new instance of DocxTextExtractor with Docx source from given
     * InputStream
     * 
     * @param f
     *            Docx file
    ...
    public DocxTextExtractor(File f) throws TextExtractorException {
        try {
            poiTextExtractor = ExtractorFactory
                    .createExtractor(new FileInputStream(f));
        } catch (IOException e) {
